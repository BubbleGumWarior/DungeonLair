...
export class SkillsComponent implements OnInit {
    ...
    async fetchSkillsList(characterName: string) {
        try {
            ...
            const skillListData = await response.json();
            this.updateSkillList(skillListData);  // Update skills with the fetched data
            await this.fetchSkillById(); // Fetch skill details after getting IDs
        } catch (error) {
    ...
    async fetchSkillById() {
        try {
            const fetchPromises = this.skillList.map(async (skillID) => {
                const response = await fetch(`http://localhost:3000/skill-list/${skillID}`);
